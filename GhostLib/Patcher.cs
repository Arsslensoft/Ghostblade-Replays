   ...
   public class Patcher
    {
        ...
        public static void GeneratePatchFile(string oldfile,string newfile, string patchfile)
        {
            try
            {


                using (FileStream fs = File.Create(patchfile))
                    Patcher.Create(File.ReadAllBytes(oldfile), File.ReadAllBytes(newfile), fs);

              
            }
            catch (Exception ex)
            {
                Logger.Instance.Log.Error("Failed to patch file " + newfile, ex);
            }
        }
        public static void PatchFile(string file,string patchfile)
        {
            try
            {
                ...
                if (File.Exists(file + ".patched"))
                    File.Delete(file + ".patched");


                using (FileStream fs = File.Open(tempfile, FileMode.Open))
                using (FileStream pfs = File.Create(file+".patched"))
                    Patcher.Apply(fs, () => new FileStream(patchfile, FileMode.Open, FileAccess.Read, FileShare.Read), pfs);
            ...
            {
                Logger.Instance.Log.Error("Failed to patch file " + file, ex);
            }
        }
