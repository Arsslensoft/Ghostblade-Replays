...
namespace RtmpSharp.IO.ObjectWrappers
{
    class BasicObjectWrapper : IObjectWrapper
    {
        ...
        class BasicMemberWrapper : IMemberWrapper
        {
            ...
            {
                var attribute = memberInfo.GetCustomAttributes(typeof(SerializedNameAttribute),true);
                serializedName = attribute != null ? ((SerializedNameAttribute)attribute[0]).SerializedName : name;
            }

            public object GetValue(object instance)
            {
                return isField ? fieldInfo.GetValue(instance) : propertyInfo.GetValue(instance, new object[1]{null});
            }
