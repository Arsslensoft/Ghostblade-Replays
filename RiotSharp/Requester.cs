...
namespace RiotSharp
{
    class Requester
    {
        ...
        protected HttpWebRequest PrepareRequest(string relativeUrl, List<string> addedArguments)
        {
            ...
            {
                if(rootDomain == "status.leagueoflegends.com")
                    return request = (HttpWebRequest)WebRequest.Create(string.Format("http://{0}{1}?api_key={2}"
                       , rootDomain, relativeUrl, ApiKey));

                if (rootDomain == "na.lolesports.com")
                    return request = (HttpWebRequest)WebRequest.Create(string.Format("http://{0}{1}"
                       , rootDomain, relativeUrl));


                if (relativeUrl.Contains("/observer-mode/rest/consumer/getSpectatorGameInfo"))
                {
