...
namespace RtmpSharp.IO
{
    class SerializerObjectFactory
    {
        ...
        static readonly Type[] BaseSerializableTypes = typeof(SerializerObjectFactory).Assembly
            .GetTypes()
            .Where(x => x.GetCustomAttributes(typeof(SerializableAttribute),true).Any())
            .ToArray();
        ...
        public void Register(Type type)
        {
            ...
            var attributes = type.GetCustomAttributes(typeof(SerializedNameAttribute),false).ToArray();
            if (attributes.Length == 0)
            {
                remoteToLocalNames[type.FullName] = type;
            }
            else if (attributes.Count(x => ((SerializedNameAttribute)x).Canonical) == 1)
            {
                foreach (var attribute in attributes)
                    this.RegisterAlias(type, ((SerializedNameAttribute)attribute).SerializedName, ((SerializedNameAttribute)attribute).Canonical);
            }
            ...
            else
            {
                var message = string.Format(
                    ...
                    type.FullName,
                    attributes.Count(x => ((SerializedNameAttribute)x).Canonical));
